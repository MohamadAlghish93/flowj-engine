...
public class JwtEmailAndPasswordAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
        ...
        String token = Jwts.builder()
                ...
                // This is important because it affects the way we get them back com the Gateway.
                .claim("authorities", auth.getAuthorities().stream()
                        .map(GrantedAuthority::getAuthority).collect(Collectors.toList()))
                .setIssuedAt(new Date(now))
                .setExpiration(new Date(now + jwtConfig.getExpiration() * 1000))  // com milliseconds
        ...
        responseService.setData(jwtConfig.getPrefix() + token);
        Gson gson = new Gson();
        response.getWriter().write(gson.toJson(responseService) );
